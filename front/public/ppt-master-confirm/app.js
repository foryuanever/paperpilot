...
(function () {
    ...
    var BRIDGE_KEY = new URLSearchParams(window.location.search).get("bridge") || "";
    var BRIDGE_RECOMMENDATIONS = new URLSearchParams(window.location.search).get("recommendations") || "";
    var BRIDGE_PREFIX = "papersolver-ppt-master-confirm:";

    function decodeBridgeRecommendations() {
        if (!BRIDGE_RECOMMENDATIONS) return null;
        try {
            var binary = window.atob(BRIDGE_RECOMMENDATIONS);
            var escaped = Array.prototype.map.call(binary, function (char) {
    ...
    function bridgeJsonResponse(value, status) {
    ...
    function bridgeFetch(url, options) {
        if (!BRIDGE_KEY) return window.fetch(url, options);
        var requestUrl = String(url || "");
        if (requestUrl === "/api/catalogs" || requestUrl === "/static/catalogs.json") {
            return window.fetch("./catalogs.json", options);
        }
        if (requestUrl === "/api/recommendations") {
            try {
                var embedded = decodeBridgeRecommendations();
                if (embedded) return bridgeJsonResponse(embedded);
                var stored = window.localStorage.getItem(BRIDGE_PREFIX + BRIDGE_KEY + ":recommendations");
                return bridgeJsonResponse(stored ? JSON.parse(stored) : {}, stored ? 200 : 404);
            } catch (error) {
                return bridgeJsonResponse({ error: String(error) }, 500);
            }
        }
        if (requestUrl === "/api/confirm") {
            try {
                var payload = JSON.parse((options && options.body) || "{}");
                window.localStorage.setItem(BRIDGE_PREFIX + BRIDGE_KEY + ":result", JSON.stringify(payload));
                if (window.opener) {
                    window.opener.postMessage({
                        type: "papersolver:ppt-master-confirmed",
        ...
        if (requestUrl === "/api/shutdown") return bridgeJsonResponse({ ok: true });
    ...
    function normalizePageCount(value) {
        ...
        var first = Math.max(1, Math.min(10, parseInt(numbers[0], 10) || 10));
        var last = Math.max(first, Math.min(10, parseInt(numbers[numbers.length - 1], 10) || first));
    ...
    var MESSAGES = {
        en: {
            ...
            placeholder_pages: "Up to 10 slides, e.g. 8 or 10",
        ...
        zh: {
            ...
            placeholder_pages: "最多 10 张，如：8 或 10",
    ...
    function textField(parent, getVal, setVal, placeholderKey, numeric) {
        ...
        input.addEventListener("input", function () {
            ...
            var bounded = Math.max(1, Math.min(10, parsed));
            input.value = String(bounded);
            setVal(String(bounded));
        });
        input.addEventListener("blur", function () {
            if (!numeric || !String(input.value || "").trim()) return;
            var bounded = Math.max(1, Math.min(10, parseInt(input.value, 10) || 10));
            input.value = String(bounded);
            setVal(String(bounded));
        });
    ...
    function renderPages(host) {
        ...
        textField(sec, function () { return STATE.page_count; },
            function (v) { STATE.page_count = normalizePageCount(v); }, "placeholder_pages", true);
        host.appendChild(sec);
    }
    ...
    function initTier2State() {
        STATE.page_count = normalizePageCount((REC.page_count && REC.page_count.value != null) ? REC.page_count.value : (STATE.page_count || ""));
    ...
    function pollForTier2() {
        bridgeFetch("/api/recommendations", { cache: "no-store" })
    ...
    function confirm() {
        ...
        payload.page_count = normalizePageCount(payload.page_count);
        payload.stage = "final";
    ...
    function loadCatalogs() {
        return bridgeFetch("/api/catalogs")
            .then(function (r) { if (r.ok) return r.json(); throw new Error("no api"); })
            .catch(function () { return bridgeFetch("/static/catalogs.json").then(function (r) { return r.json(); }); });
    }
    ...
    function boot() {
        ...
        Promise.all([
            loadCatalogs(),
            bridgeFetch("/api/recommendations").then(function (r) { if (!r.ok) throw new Error("load failed"); return r.json(); })
        ]).then(function (res) {
            CAT = res[0];
            REC = res[1];
