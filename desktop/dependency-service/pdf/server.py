...
# Large PDFs are translated through a single desktop bridge so public engines
# are not rate-limited. A fixed 15-minute total timeout incorrectly kills a
# healthy 10+ page translation near the final batches. Keep a finite overall
# limit, but fail early only when the bridge has made no progress at all.
...
def run_translate_task(task_id, data):
    ...
    try:
        ...
        # Four workers shorten long documents while keeping PDFium/layout work
        # bounded. The previous single-worker path made long PDFs appear frozen.
        try:
        ...
        def page_progress(progress):
            ...
            # pdf2zh calls back before processing the current page.
        ...
        if task_failed(task_id):
            raise RuntimeError("翻译任务已停止")
        update_task(task_id, progress=96, message="翻译完成，正在校验并保存双语 PDF", lastActivityAt=time.time())
...
def install_papersolver_google_bridge():
    ...
    if getattr(translator_module, "_papersolver_google_bridge_installed", False):
        return
    # pdf2zh's converter otherwise retries every failed paragraph forever.
    # Limit this module's decorator, without modifying other tenacity users.
    from tenacity import stop_after_attempt
    original_retry = converter_module.retry
    def bounded_retry(*args, **kwargs):
        kwargs["stop"] = stop_after_attempt(2)
        kwargs["reraise"] = True
        return original_retry(*args, **kwargs)
    converter_module.retry = bounded_retry
    ...
    class PaperSolverGoogleTranslator(translator_module.BaseTranslator):
        ...
        def __init__(self, lang_in, lang_out, model, ignore_cache=False, **kwargs):
            ...
            # pdf2zh calls the same translator for the document. Once the
            # bibliography heading is reached, every following fragment must
            # remain verbatim, including entries without a DOI or URL.
            self.bibliography_mode = False

        def do_translate(self, text):
            # pdf2zh invokes this method in its worker pool. threading.local
            # from the parent task is not inherited by those workers.
            translation_context.task_id = self.task_id
            if not hasattr(translation_context, "http_session"):
                translation_context.http_session = requests.Session()
            if task_failed(self.task_id):
                raise RuntimeError("翻译任务已停止，请重试")
            ...
            touch_translate_task()
            response = translation_context.http_session.post(
                self.endpoint,
                json={
                    # pdf2zh exposes GoogleTranslator as its layout-preserving
                    # translator. PaperSolver routes this local bridge to the
                    # currently selected desktop translation provider.
            ...
            if not result:
                ...
                mark_bridge_failure(message)
                raise RuntimeError(message)
            translated = sanitize_bridge_translation(
                original,
                str(result),
                protected_layout_tokens,
                bridge_source_lang,
            )
...
def clean_bridge_text(value):
    text = str(value or "")
    # Remove PUA Unicode characters, replacement chars, and decorative black square/bullet glyphs that cause black boxes
    text = re.sub(r"[\ue000-\uf8ff\ufffd\u25a0\u25aa\u25cf\u25c6\u25b2\u25b6\uf0a7\uf0b7]", " ", text)
    # Strip out leaked document timestamps and chunk markers in brackets: e.g. 【2019-12-26 00:55:56】, 【038】, etc.
    ...
    # Keep line boundaries. Flattening pdf2zh's fragments changes paragraph and
    # formula layout, which can make a valid translation overlap on the page.
...
def sanitize_bridge_translation(original, value, tokens, source_lang):
    """Reject provider garbage before pdf2zh writes it into a reconstructed PDF."""
    ...
    # Clean any leaked bracketed timestamps, page/block markers, and PUA dingbat boxes
    ...
    if source_lang not in {"", "auto"} and non_latin_source and len(source) >= 24 and source == result:
        raise RuntimeError("翻译服务返回原文，已停止重建 PDF")
...
def guess_source_language(value, requested):
    """Give short non-English fragments a stable source language hint."""
    ...
    if sum(len(re.findall(rf"\b{word}\b", text)) for word in ("il", "gli", "una", "che", "per", "con")) >= 2:
        return "it"
    if re.search(r"[ãõáéíóúâêôç]", text) or sum(len(re.findall(rf"\b{word}\b", text)) for word in ("uma", "uma", "dos", "das", "para", "com", "não")) >= 2:
        return "pt"
    return "auto"
...
def is_protected_bibliography_text(value):
    ...
    if compact in {
        ...
        return True
    # If text is a typical bibliography entry (e.g. starting with [1], 1., author names + year, DOI, URL, arXiv, etc.)
...
def is_bibliography_heading(value):
    """Recognize a bibliography heading before pdf2zh splits its entries."""
    ...
    return bool(re.match(
        r"^(?:\d+\.?\s*)?(?:references?|bibliography|works\s+cited|literature\s+cited|参考文献|参考资料|references\s*and\s*notes)(?:\s*[:：.。]|\s+|$)",
        raw,
        re.IGNORECASE,
    ))
...
def touch_translate_task(message=None):
    ...
    # pdf2zh does not expose a reliable total fragment count. Keep progress
    # conservative and monotonic, and show the exact activity count in the
    # status message so 32% cannot masquerade as a hung task.
    updates = {
        "lastActivityAt": time.time(),
